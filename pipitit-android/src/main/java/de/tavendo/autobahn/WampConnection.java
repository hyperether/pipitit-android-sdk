...
public class WampConnection extends WebSocketConnection implements Wamp {
    ...
    private static final char[] mBase64Chars =
    ...
    private final ConcurrentHashMap<String, CallMeta> mCalls =
            new ConcurrentHashMap<String, CallMeta>();
    ...
    private final ConcurrentHashMap<String, SubMeta> mSubs =
            new ConcurrentHashMap<String, SubMeta>();
    ...
    protected void createWriter() {
        ...
        mWriterThread.start();
        mWriter = new WampWriter(mWriterThread.getLooper(), mMasterHandler, mTransportChannel,
                mOptions, mSSLEngine);

        PipititLogger.d(TAG, "writer created and started");
    }
    ...
    protected void createReader() {
        mReader = new WampReader(mCalls, mSubs, mMasterHandler, mTransportChannel, mOptions,
                "AutobahnReader", mSSLEngine);
        mReader.start();

        PipititLogger.d(TAG, "reader created and started");
    }
    ...
    public void connect(String wsUri, Wamp.ConnectionHandler sessionHandler, WampOptions options,
        ...
        try {
            connect(wsUri, new String[]{"wamp"}, new WebSocketConnectionHandler() {
                ...
                public void onOpen() {
                    if (mSessionHandler != null) {
                        ...
                        PipititLogger.d(TAG, "could not call onOpen() .. handler already NULL");
                    }
                }
                ...
                public void onClose(int code, String reason) {
                    if (mSessionHandler != null) {
                        ...
                        PipititLogger.d(TAG, "could not call onClose() .. handler already NULL");
                    }
                }
            ...
            if (mSessionHandler != null) {
                mSessionHandler.onClose(WebSocketConnectionHandler.CLOSE_CANNOT_CONNECT,
    ...
    protected void processAppMessage(Object message) {
        ...
        if (message instanceof WampMessage.CallResult) {
            ...
            PipititLogger.d(TAG,
                    "WAMP session " + welcome.mSessionId + " established (protocol version " +
                            welcome.mProtocolVersion + ", server " + welcome.mServerIdent + ")");

        } else {
            PipititLogger.d(TAG, "unknown WAMP message in AutobahnConnection.processAppMessage");
        }
    }
     ...
     * @param arguments Call arguments.
     */
    public void call(String procUri, Class<?> resultType, CallHandler resultHandler,
                     Object... arguments) {
     ...
     * @param arguments Call arguments.
     */
    public void call(String procUri, TypeReference<?> resultType, CallHandler resultHandler,
                     Object... arguments) {
    ...
    private void subscribe(String topicUri, SubMeta meta) {
        ...
        if (!mSubs.containsKey(uri)) {
            ...
            WampMessage.Subscribe msg =
                    new WampMessage.Subscribe(mOutgoingPrefixes.shrink(topicUri));
            mWriter.forward(msg);
        }
    }
    ...
    public void publish(String topicUri, Object event) {
        ...
        WampMessage.Publish msg =
                new WampMessage.Publish(mOutgoingPrefixes.shrink(topicUri), event);
        mWriter.forward(msg);
    }
}
