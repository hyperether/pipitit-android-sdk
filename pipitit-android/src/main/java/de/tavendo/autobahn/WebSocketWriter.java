...
public class WebSocketWriter extends Handler {
     ...
     * @param sslengine
     */
    public WebSocketWriter(Looper looper, MasterHandler master, SocketChannel socket,
        ...
        if (mSSLEngine != null) {
            mBufferEnc = new ByteBufferOutputStream(options.getMaxFramePayloadSize() + 14 + 100,
                    4 * 64 * 1024, true);
            mBuffer =
                    new ByteBufferOutputStream(options.getMaxFramePayloadSize() + 14, 4 * 64 * 1024,
                            false);
        } else {
            mBufferEnc = null;
            mBuffer =
        ...
        if (mSSLEngine != null) {
            PipititLogger.d(TAG, "WS writer created [WSS]");
        } else {
            PipititLogger.d(TAG, "WS writer created [WS]");
        }
    }
     ...
     * underlying TCP.
     *
     * @param message Message to send to WebSockets writer. An instance of the message classes
     * inside WebSocketMessage or another type which then needs to be handled within
    ...
    protected void sendFrame(int opcode, boolean fin, byte[] payload, int offset, int length)
            throws IOException {
        PipititLogger.d(TAG, "SEND - " + ByteUtils.byteArrayToString(payload));
        // first octet
        byte b0 = 0;
        if (fin) {
            b0 |= (byte) (1 << 7);
        ...
        try {
            ...
            while (mBuffer.remaining() > 0 || trigger) {
                ...
                if (mSSLEngine != null) {
                    ...
                    SSLEngineResult res =
                            mSSLEngine.wrap(mBuffer.getBuffer(), mBufferEnc.getBuffer());
                    ...
                    PipititLogger.d(TAG, "WRITTEN (WS): " + written);
                }
            }
     ...
     *
     * @param msg An instance of the message types within WebSocketMessage or a message that is
     * handled in processAppMessage().
     */
