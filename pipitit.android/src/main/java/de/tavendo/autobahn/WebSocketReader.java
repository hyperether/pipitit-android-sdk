...
import com.hyperether.pipitit.util.ByteUtils;
...
public class WebSocketReader extends Thread {
     ...
     * @param sslengine
     */
    public WebSocketReader(MasterHandler master, SocketChannel socket, WebSocketOptions options,
                           String threadName, SSLEngine sslengine) {
        ...
        if (mSSLEngine != null) {
            ...
            mFrameBuffer = ByteBuffer.allocate(options.getMaxFramePayloadSize() + 14);
        } else {
            mBufferEnc = null;
    ...
    private boolean processData() throws Exception {
        ...
        if (mFrameHeader == null) {
            ...
            if (mFrameBuffer.position() >= mFrameHeader.mTotalLen) {
                ...
                PipititLogger.d(TAG, "RECEIVE - " + ByteUtils.byteArrayToString(framePayload));
                if (mFrameHeader.mOpcode > 7) {
                    ...
                    if (framePayload != null) {
                        ...
                        if (mMessagePayload.size() + framePayload.length >
                                mOptions.getMaxMessagePayloadSize()) {
     ...
     * HAS NOT been set.
     *
     * @param payload Text message payload as Java String decoded from raw UTF-8 payload or null
     * (empty payload).
     */
    ...
    private Map<String, String> parseHttpHeaders(byte[] buffer)
        ...
        for (String line : lines) {
            if (line.length() > 0) {
                ...
                if (h.length == 2) {
                    headers.put(h[0], h[1]);
                    PipititLogger.w(TAG, String.format("'%s'='%s'", h[0], h[1]));
                }
            }
        }
    ...
    private Pair<Integer, String> parseHttpStatus() throws UnsupportedEncodingException {
        ...
        PipititLogger.w(TAG, String.format("Status: %d (%s)", statusCode, statusMessage));
    ...
    private void runDelegatedTasks(SSLEngineResult result) throws Exception {
        ...
        PipititLogger.d(TAG, "SSL result: " + result.toString());

        if (result.getHandshakeStatus() == HandshakeStatus.NEED_TASK) {
            Runnable runnable;
            while ((runnable = mSSLEngine.getDelegatedTask()) != null) {
                PipititLogger.d(TAG, "running SSL delegated task");
                runnable.run();
            }
        ...
        try {
            ...
            do {
                ...
                if (mSSLEngine != null) {
                    ...
                    PipititLogger.d(TAG,
                            "READ (WSS): " + len + " - " + mBufferEnc.remaining() + " - " +
                                    mSocket.isBlocking());
                } else {
                    len = mSocket.read(mFrameBuffer);
                    PipititLogger.d(TAG, "READ (WS): " + len);
                }
                ...
                if (len > 0) {
                    ...
                    if (mSSLEngine != null) {
                        ...
                        do {
                            PipititLogger.d(TAG,
                                    "before UNWRAP: " + mFrameBuffer.position() + " - " +
                                            mFrameBuffer.limit() + " - " +
                                            mFrameBuffer.remaining() + " - " + mFrameBuffer.mark() +
                                            " - " + mFrameBuffer.capacity());
                            res = mSSLEngine.unwrap(mBufferEnc, mFrameBuffer);

                            PipititLogger.d(TAG, "res Status " + res.getStatus());

                            PipititLogger.d(TAG, "res HS Status " + res.getHandshakeStatus());

                            PipititLogger.d(TAG, "HS Status " + mSSLEngine.getHandshakeStatus());

                            runDelegatedTasks(res);
                            PipititLogger.d(TAG,
                                    "after  UNWRAP: " + mFrameBuffer.position() + " - " +
                                            mFrameBuffer.limit() + " - " +
                                            mFrameBuffer.remaining() + " - " + mFrameBuffer.mark() +
                                            " - " + mFrameBuffer.capacity());
                        ...
                        if (mSSLEngine.getHandshakeStatus() ==
                                SSLEngineResult.HandshakeStatus.NEED_WRAP ||
                                mMaster.getWriterHasData()) {
                            PipititLogger.d(TAG, "NEED_WRAP => trigger");
                            notify(new WebSocketMessage.TriggerWrite());
                        } else {
                            PipititLogger.d(TAG, "NO NEED_WRAP");
                        }
                    }
                    ...
                    PipititLogger.d(TAG, "run() : ConnectionLost");
            ...
            PipititLogger.d(TAG, "run() : WebSocketException (" + e.toString() + ")");
            ...
            PipititLogger.d(TAG, "run() : SocketException (" + e.toString() + ")");
            ...
            PipititLogger.d(TAG, "run() : Exception (" + e.toString() + ")");
